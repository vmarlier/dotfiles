  ...
  if handle then
    for dir in handle:lines() do
      ...
      if repo_name then
        ...
        local item_action = string.format(":lua vim.cmd('cd %s'); vim.cmd('terminal'); vim.cmd('NvimTreeToggle');",
          trimmed_dir)
...
starter.setup({
  ...
  items = {
    ...
    { name = "Dotfiles", action = ":lua vim.cmd('cd ~/Git/valentin.marlier/dotfiles'); vim.cmd('terminal'); vim.cmd('NvimTreeToggle');", section = 'Perso' },
    pleo_repo,
